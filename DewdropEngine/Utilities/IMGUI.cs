...
// stolen from https://github.com/PascalPieper/Portfolio_TileGame/blob/master/TileGame/Utility/SfmlExtension/GuiImpl.cs
public class ImGuiSfml {
	static bool _windowHasFocus;
	static readonly bool[] MousePressed = {
		false, false, false
	};
	static readonly bool[] TouchDown = {
		false, false, false
	};
	static Texture _fontTexture;
	static readonly Cursor[] MouseCursors = new Cursor[(int)ImGuiMouseCursor.COUNT];
	...
	static void OnMouseButtonPressed (object sender, MouseButtonEventArgs args) {
	...
	static void OnMouseButtonReleased (object sender, MouseButtonEventArgs args) {
		var button = (int)args.Button;
		if (button >= 0 && button < 3) {
	...
	static void OnMouseWheelScrolled (object sender, MouseWheelScrollEventArgs args) {
		var io = ImGui.GetIO();
		switch (args.Wheel) {
	...
	static void OnTextEntered (object sender, TextEventArgs args) {
		var io = ImGui.GetIO();
	...
	static void OnGainedFocus (object sender, EventArgs args) {
		_windowHasFocus = true;
	}

	static void OnLostFocus (object sender, EventArgs args) {
	...
	static extern void RenderDrawLists (ImDrawDataPtr drawData, ImGuiIOPtr io);
	...
	static unsafe uint ConvertImTextureIdToGlTextureHandle (IntPtr imTexId) {
	...
	static string ClipboadText {
		get => Clipboard.Contents;
		set => Clipboard.Contents = value;
	}

	static void LoadMouseCursor (ImGuiMouseCursor imguiCursorType, Cursor.CursorType sfmlCursorType) {
		MouseCursors[(int)imguiCursorType] = new Cursor(sfmlCursorType);
	}

	static void UpdateMouseCursor (Window window) {
		var io = ImGui.GetIO();
	...
	static void Image (Texture texture, Vector2f size, Color tintColor, Color borderColor) {
	...
	static void Image (Texture texture, FloatRect textureRect, Color tintColor, Color borderColor) {
		Image(texture, new Vector2f(Math.Abs(textureRect.Width), Math.Abs(textureRect.Height)),
			textureRect, tintColor, borderColor);
	}

	static void Image (Texture texture, Vector2f size, FloatRect textureRect, Color tintColor,
	                   Color borderColor) {
	...
	static void Image (Sprite sprite, Color tintColor, Color borderColor) {
		var bounds = sprite.GetGlobalBounds();
		Image(sprite, new Vector2f(bounds.Width, bounds.Height), tintColor, borderColor);
	}

	static void Image (Sprite sprite, Vector2f size, Color tintColor, Color borderColor) {
	...
	static bool ImageButton (Texture texture, int framePadding, Color bgColor, Color tintColor) {
		return ImageButton(texture, new Vector2f(texture.Size.X, texture.Size.Y), framePadding, bgColor, tintColor);
	}

	static bool ImageButton (Texture texture, Vector2f size, int framePadding, Color bgColor,
	                         Color tintColor) {
		var textureSize = texture.Size;
	...
	static bool ImageButton (Sprite sprite, int framePadding, Color bgColor, Color tintColor) {
	...
	static bool ImageButton (Sprite sprite, Vector2f size, int framePadding, Color bgColor, Color tintColor) {
		var texture = sprite.Texture;
		if (texture == null) {
			return false;
		}
	...
	static void DrawLine (Vector2f a, Vector2f b, Color col, float thickness = 1.0f) {
	...
	static void DrawRect (FloatRect rect, Color color, float rounding = 0.0f, int roundingCorners =
