...
public class RenderPipeline {
	class IRenderableComparer : IComparer<IRenderable> {
		//used for uids
	...
	Stack<IRenderable> IRenderablesToAdd;
	Stack<IRenderable> IRenderablesToRemove;
    ...
    public void Add (IRenderable IRenderable) {
		...
		if (IRenderables.Contains(IRenderable)) {
			Outer.LogError("Tried to add IRenderable that already exists in the RenderPipeline.", null);
			return;
		}
    ...
    public void Add (BasicRenderableWrapper IRenderable) {
	    if (IRenderables.Contains(IRenderable.Renderable)) {
		    Outer.LogError("Tried to add IRenderable that already exists in the RenderPipeline.", null);
		    return;
	    }
	...
	public void Draw () {
		...
		viewRect.Left = _view.Center.X - _view.Size.X/2f;
		viewRect.Top = _view.Center.Y - _view.Size.Y/2f;
		viewRect.Width = _view.Size.X;
		viewRect.Height = _view.Size.Y;
